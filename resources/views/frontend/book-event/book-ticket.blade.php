        ...
        <div class="modal-dialog modal-cstm" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    <div class="row">
                        <div class="col-md-4 payment_details_div pr-0">
                            <div class="px-4 pt-4">
                            ...
                            <div class="girl_img">
                                <img src="{{asset('images/book_img.png')}}" class="img-fluid w-auto" alt="girl-bg">
                            </div>
                        </div>
                ...
                <div class="modal-footer p-0">
                    <button type="button" class="btn btn-primary w-100 py-3 modal-footer-btn" id="modalSubmitBtn" disabled>
                        Submit Now
                    </button>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        let currentTicketCount = parseInt("{{ $bookingData['total_player_forms'] }}");
        const isDoubles = ("{{ $bookingData['category'] }}" === 'Doubles');
        const playersPerTicket = isDoubles ? 2 : 1;
        const ticketPrice = parseFloat("{{ $packageDetails['ticket'] }}");
        const taxAmount = parseFloat("{{ $settingDetails['tax'] }}");
        const initialQuantity = parseInt("{{ $bookingData['quantity'] }}");
        ...
        function renderTicketForm(ticketNumber) {
            ...
            // Calculate which ticket this player belongs to
            const ticketIndex = Math.ceil(ticketNumber / playersPerTicket);
            
            return `
                <div class="playerGroup" id="ticketGroup_${ticketNumber}">
                    <button type="button" class="remove-ticket-btn" onclick="removeTicket(${ticketNumber})" ${ticketNumber <= initialQuantity * playersPerTicket ? 'disabled style="display:none;"' : ''}>
                        <i class="fas fa-times-circle"></i>
                    </button>
                    
                    <h4 class="mb-3 text-center ticketHeading">
                        Ticket ${ticketIndex} ${isDoubles ? 'Player ' + ((ticketNumber % playersPerTicket) + 1) : ''}
                    </h4>
                    ...
                    <div class="playerForm">
                        <div class="row">
                            <div class="mb-3 col-lg-4">
                                ...
                                <input type="text" class="form-control" name="player_name_${ticketNumber}" id="player_name_${ticketNumber}" placeholder="Player Name" required>
                            </div>
                            
                            <div class="mb-3 col-lg-4">
                                <label for="player_contact_${ticketNumber}" class="form-label">Contact Number <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="player_contact_${ticketNumber}" id="player_contact_${ticketNumber}" placeholder="Player Contact Number" required maxlength="10" minlength="10">
                            </div>
        ...
        // Add new tickets (1 for singles, 2 for doubles)
        $('#addTicketBtn').click(function() {
            ...
            // Add the appropriate number of player forms
            for (let i = 0; i < playersPerTicket; i++) {
                currentTicketCount++;
        ...
        window.removeTicket = function(ticketNumber) {
            if (ticketNumber > initialQuantity * playersPerTicket) {
                // For doubles, remove both players of the same ticket
                const ticketIndex = Math.ceil(ticketNumber / playersPerTicket);
                const firstPlayerToRemove = (ticketIndex - 1) * playersPerTicket + 1;
                
                for (let i = 0; i < playersPerTicket; i++) {
                    $(`#ticketGroup_${firstPlayerToRemove + i}`).remove();
                    currentTicketCount--;
                }
        ...
        function updateQuantityAndPrice() {
            // Calculate the actual quantity (number of bookings)
            const actualQuantity = Math.ceil(currentTicketCount / playersPerTicket);
            
            // Update display
            $('#quantityDisplay').text(actualQuantity);
            $('#quantityText').text(actualQuantity + ' booking(s)');
            $('#total_ticket').val(actualQuantity);
            
            // Calculate new total
            const totalBeforeCoupon = (ticketPrice * actualQuantity) + taxAmount;
